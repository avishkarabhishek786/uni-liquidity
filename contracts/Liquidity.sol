...
contract LiquidityExamples is IERC721Receiver {
    ...
    mapping(address => uint[]) public depositNFTIds;
    ...
    function _createDeposit(address owner, uint _tokenId) internal {
        ...
        depositNFTIds[owner].push(_tokenId);
    }
    ...
    function getUserNftIds(address user) external view returns(uint[] memory) {
        return depositNFTIds[user];
    }

    function transferUniNFT(uint256 nftId, address to) external {
        nonfungiblePositionManager.safeTransferFrom(nonfungiblePositionManager.ownerOf(nftId), to, nftId);
    }
